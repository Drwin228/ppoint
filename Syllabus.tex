...
\lhead{}
\chead{}
\rhead{\footnotesize Math1740: Introduction to Statistics -- Spring
2023}
\lfoot{}
...
\hypersetup{breaklinks=true,
            ...
            pdfauthor={},
             pdfkeywords = {},  
            pdftitle={Math1740: Introduction to Statistics},
            colorlinks=true,
            citecolor=blue,
            urlcolor=blue,
            linkcolor=magenta,
...
\title{Math1740: Introduction to Statistics}
\date{Spring 2023}
...
\begin{multicols}{2}
  ...
  \textbf{Dr.~Francois Nguyen}\\
  Email: \href{mailto:Francois.nguyen@saintpaul.edu}{\nolinkurl{Francois.nguyen@saintpaul.edu}}\\
  Preferred contact: Email or MocroSoft TEAM\\
  Office Hours: Arrange for Zoom meeting\\
  Office: Room 3460\\
...
\#This code chunk sets up a table of meeting days for the course. \#You
can set days of the week for course meetings (e.g.~Monday and
Wednesday). \#Semester start and end dates are selected and then
university holidays like spring break are specified.
tinytex::reinstall\_tinytex() \#Days of the week that the course meets.
Must be full day name in quotes.

Days \textless- c(``Monday'', ``Wednesday'')

\#Semester start date for classes as YYYY/M/D. SemesterStart \textless-
as.Date(``2022/1/10'')

\#Semester end date for classes as YYYY/M/D. SemesterEnd \textless-
as.Date(``2022/4/29'')

\#Fall break start and stop date. FallBreak \textless- seq.Date( from =
as.Date(``2021/10/7''), to = as.Date(``2021/10/10''), by = ``day'' )

\#Spring break start and stop date. SpringBreak \textless- seq.Date(
from = as.Date(``2022/02/28''), to = as.Date(``2022/03/06''), by =
``day'' )

\#Creates a list of days within the semester. SemesterDays \textless-
seq.Date(from = SemesterStart, to = SemesterEnd, by = ``day'')

\#Add day before Thanksgiving. DayBeforeThnks\textless-as.Date(holiday(
year = getRmetricsOptions(``currentYear''), Holiday =
c(``USThanksgivingDay'')))-1

\#Full list of holidays that there are no classes including breaks.
Holidays \textless- c(as.Date(holiday( year =
getRmetricsOptions(``currentYear''), Holiday = c(``USLaborDay'',
``USThanksgivingDay'', ``USMLKingsBirthday'') )), SpringBreak,
FallBreak, DayBeforeThnks)

\#Create a list of semester days excluding holidays. SemesterDays
\textless- SemesterDays{[}!SemesterDays \%in\% Holidays{]}

\#Make dataframe with all semester days. ClassDays \textless-
data.frame(index = 1:length(SemesterDays))

\#Insert dates. ClassDays\$Date \textless- SemesterDays

\#Insert weekday designation.
ClassDays\(Weekday <- weekdays(ClassDays\)Date)

\#Limit to the class meeting days. MeetingDays \textless-
ClassDays{[}ClassDays\$Weekday \%in\% Days, {]}

\#Create numeric list of weeks. Week \textless-
cut.Date(MeetingDays\$Date, breaks = ``week'') levels(Week) \textless-
1:length(levels(Week))

MeetingDays\(Week <- Week MeetingDays\)Class \textless-
1:nrow(MeetingDays)

\#Replace this with c(``Lecture topic 1'',\ldots,``Lecture topic N'') to
fill in table.

Lecture\_Topic \textless- 1:nrow(MeetingDays)

MeetingDays \textless- bind\_cols(MeetingDays, Lecture = Lecture\_Topic)

\hypertarget{grade-table}{%
\paragraph{Grade table}\label{grade-table}}

Grades \textless- tribble( \textasciitilde{} Grade, \textasciitilde{}
Range, ``A'', ``90-100'', ``B'', ``80-89'', ``C'', ``70-79'', ``D'',
``60-69'', ``F'', ``\textless{} 60'')

\#\#\#\#Assignment weight table\#\#\#\# Weights \textless- tribble(
\textasciitilde{} Item, \textasciitilde{} Weight, ``Writing
scaffolding'', ``20 \%'', ``Semester Project'', ``25 \%'', ``Labratory
Assignments'', ``20 \%'', ``Midterm Exam'', ``15 \%'', ``Final Exam'',
``20 \%'' )
